...
export default {
  ...
  methods: {
    ...
    async updateGrids () {
      const response = await fetch('http://185.151.245.237:3000/crossword/grids')
    ...
    async pasteWord ({ word: { word }, x, y, isVertical }) {
      ...
      const url = `http://185.151.245.237:3000/crossword/clues/${this.locale}find/${word}`
      let response
    ...
    newCrossword () {
      fetch(
        'http://185.151.245.237:3000/crossword/create',
        {
          method: 'POST',
          data: {
            name: 'Test',
    ...
    async loadCrossword (id) {
      const response = await fetch(`http://185.151.245.237:3000/crossword/${id}`)
        .catch(console.log)
    ...
    async saveGrid () {
      ...
      const response = await fetch(
        'http://185.151.245.237:3000/crossword/grids',
        {
          method: 'POST',
          headers: {
    ...
    getSuggestions (queries, useCache = true) {
      return Promise.all(queries.map(async (query) => {
        ...
        let response
        const url = `http://185.151.245.237:3000/crossword/words/${this.locale}find/${query}`
    ...
    getSuggestionCounts (queries, useCache = true) {
      return Promise.all(queries.map(async (query) => {
        ...
        let response
        const url = `http://185.151.245.237:3000/crossword/words/${this.locale}count/${query}`
